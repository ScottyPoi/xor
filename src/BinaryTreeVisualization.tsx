...
const generateTreeData = (depth: number, width: number, height: number): ITreeNode => {
  const rootNodePosition = { x: width / 2, y: 100 };
  ...
  return (
    <div className="binary-tree-container">
      <div className="controls">
        <button className="button" onClick={() => setDepth(depth + 1)}>Increase Depth</button>
        <span>Depth: {depth}</span>
        <button className="button" onClick={() => setDepth(Math.max(depth - 1, 1))}>Decrease Depth</button>
      </div>
